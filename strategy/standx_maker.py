        ...
        while self._running:
            ...
            try:
                ...
                for o in self.open_orders['bid']:
                    ...
                    logger.info(f"当前买单: {o['id']}, 价格：{o['price']}, bps：{delta_bps:.4f}")
                for o in self.open_orders['ask']:
                    delta_bps = abs(o['price'] - self.current_price) / self.current_price * 10000
                    logger.info(f"当前卖单: {o['id']}, 价格：{o['price']}, bps：{delta_bps:.4f}")
                logger.info(f"当前价格：{self.current_price}, 持仓：{self.position_qty}, ATR：{self.current_atr:.2f}, move: {self.detector.held_danger:.2f}, 波动检查状态: {self.detector.state}")
                logger.info("-------------------")
            except Exception as exc:
...
async def main():
    ...
    from common.config import (
        STANDX_MAKER_API_TOKEN,
        STANDX_MAKER_ED25519_PRIVATE_KEY
    )
    
    # 检查是否使用 API TOKEN 方式
    api_token = STANDX_MAKER_API_TOKEN
    ed25519_private_key_env = STANDX_MAKER_ED25519_PRIVATE_KEY
    
    if api_token:
        # 使用 API TOKEN 方式
        logger.info("使用 API TOKEN 方式启动...")
        
        # 如果提供了 ED25519 私钥，则使用它；支持 hex 或 base58
        ed25519_private_key_bytes = None
        if ed25519_private_key_env:
            try:
                ed25519_private_key_bytes = base58.b58decode(ed25519_private_key_env)
                logger.debug("使用自定义 ED25519 私钥 (base58)")
            except Exception as e:
                logger.error(f"解析 ED25519 私钥失败: {e}")
