...
class StandXAdapter(ExchangeInterface):
    ...
    def __init__(
        ...
        api_token: str = None,
        ed25519_private_key: bytes = None
    ):
        ...
        # Handle API token and Ed25519 private key
        self.api_token = api_token or os.getenv('STANDX_API_TOKEN', '')
        
        # Handle Ed25519 private key - can be provided directly or generated
        if ed25519_private_key:
            self.ed25519_private_key = ed25519_private_key
            # Extract public key from provided private key
            private_key_obj = ed25519.Ed25519PrivateKey.from_private_bytes(ed25519_private_key)
            self.ed25519_public_key = private_key_obj.public_key().public_bytes(
                encoding=serialization.Encoding.Raw,
                format=serialization.PublicFormat.Raw
            )
            import base58
            self.request_id = base58.b58encode(self.ed25519_public_key).decode("utf-8")
        else:
    ...
    async def _ensure_authenticated(self) -> bool:
        ...
        if self.api_token:
            success, error = await self.create_auth_token_with_api_key()
            if success:
                return True
            else:
                logger.warning(f"API token authentication failed: {error}, trying wallet authentication...")
    ...
    async def create_auth_token_with_api_key(self) -> Tuple[str, str]:
        """
        Create authentication token using API TOKEN directly.
        This method bypasses the wallet signature flow and uses the provided API token.
        ...
        try:
            ...
            logger.error(f"create_auth_token_with_api_key error: {e}")
            return "", str(e)

    async def create_auth_token(self) -> Tuple[str, str]:
        """
        Create authentication token using StandX wallet signature flow.
        This method uses the traditional wallet private key authentication.
        For API token authentication, use create_auth_token_with_api_key().
        Returns: (auth_token, error)
        """
        try:
            # If we already have a valid token, return it
            ...
            # Check if we have the necessary credentials for wallet authentication
            if not self.private_key or not self.wallet_address:
                return "", "Wallet private key and address required for wallet authentication"
            ...
            logger.info("Successfully created authentication token using wallet signature")
...
async def test_api_token_authentication():
    """Test the new API token authentication flow."""
    print("=== Testing StandX API Token Authentication Flow ===")
    
    # Test with API token only (no wallet private key)
    adapter = StandXAdapter(
        chain="bsc",
        api_token=os.getenv('STANDX_API_TOKEN', 'test_api_token_12345')
    )
    
    try:
        print(f"1. StandX Adapter initialized with API token authentication")
        print(f"   API Token: {adapter.api_token[:20]}... (length: {len(adapter.api_token)})")
        print(f"   Ed25519 requestId: {adapter.request_id}")
        
        # Test API token authentication
        print("2. Testing API token authentication...")
        token, error = await adapter.create_auth_token_with_api_key()
        if error:
            print(f"   API token authentication failed: {error}")
            # This is expected if no real API token is provided
            print("   Note: This is expected if no real API token is provided in environment")
        else:
            print(f"   Successfully authenticated using API token: {token[:50]}...")
        
        # Test that the adapter can fall back to wallet authentication if API token fails
        print("3. Testing fallback to wallet authentication...")
        # This would require wallet credentials, so we'll just test the logic
        
        # Test body signature generation (should work with provided Ed25519 key)
        print("4. Testing body signature generation with custom Ed25519 key...")
        test_payload = {"test": "data", "symbol": "BTC-USD"}
        body_signature = await adapter._generate_body_signature(test_payload)
        print(f"   Generated body signature headers: {list(body_signature.keys())}")
        print(f"   Signature: {body_signature['x-request-signature'][:50]}...")
        
        print("\n=== API Token Authentication Flow Test Completed ===")
    finally:
        await adapter.close()
...
async def main():
    ...
    # Run API token authentication test
    await test_api_token_authentication()
